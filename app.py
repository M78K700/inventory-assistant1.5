...
def show_inventory_page():
    ...
    display_chatbot(st.session_state.user_id)
...
def display_chatbot(user_id):
    """Display chatbot with inventory management capabilities"""
    st.markdown("---")
    ...
    with st.form(key='chat_form'):
        user_input = st.text_input("Ask me anything about your inventory or use commands (e.g., 'add 5 apples', 'remove 2 milk', 'status')")
    ...
    if submit_button and user_input:
        ...
        if inventory:
            ...
            try:
                response = get_chatbot_response(user_id, user_input, inventory_df)
            except Exception as e:
            ...
            st.info("No inventory data available for the assistant")
